  ...
  if (senderID == global.data.botID) return;
    ...
    if(event.type == "message_unsend") {
     ...
     let name = await Users.getNameUser(senderID);
      if(getMsg.attachment[0] == undefined) return api.sendMessage(`${name} just unsent this message : ${getMsg.msgBody}`,threadID)
      else {
            let num = 0
            let msg = {
              body:`${name} just unsent this ${getMsg.attachment.length} attachments : ${(getMsg.msgBody != "") ? `\ncontent : ${getMsg.msgBody}` : ""}`,
              attachment:[],
...
module.exports.run = async function ({ api, event, Threads }) {
    ...
    const status = data["resend"] ? "turned on" : "turned off";
    return api.sendMessage(`Resend feature is now ${status} for this thread.`, threadID);
}    