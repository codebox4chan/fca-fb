...
this.run = async function ({ args, api, event }) {
  ...
  if (!vhks || !hks) {
    return send(`HuyKaiserDev Check Minecraft Status Server\nUsage: ${this.config.usages}`, event.threadID);
  }
  ...
  try {
    switch (vhks) {
      ...
      case 'bedrock':
        ...
        break;
      default:
        return send("Vui Lòng Chọn Server [ Java/BedRock ]", event.threadID);
    }
    ...
    send(
      `HuyKaiserDev Check Minecraft Status Server\nMinecraft: [ Java/Bedrock ]\nOnline: ${online}\nServer: ${host}:${port}\nHost: ${host}\nPort: ${port}\nActive Players: ${a}\nMax Players: ${b}\nServer Name: ${c}\nVersion: ${serverVersion.name}\nGamemode: ${d}`,
      event.threadID
    );
  } catch (error) {
