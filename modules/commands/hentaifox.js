...
module.exports.config = {
  ...
  version: "1.0.0",
  hasPermssion: 0,
  credits: "Minn", //do not change please:((
...
module.exports.run = async function({ api, event,args }) {
  try {
    ...
    const dname = `cache/hentaifox.${uuidv4()}.png`;
    ...
    const msg = `${page}/${totalpages}`; api.sendMessage({body:msg,attachment:fs.createReadStream(path.join(imgpath))},event.threadID);
  } catch (error) {
    console.error(error);
