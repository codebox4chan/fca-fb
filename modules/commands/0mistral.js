...
module.exports.config = {
  ...
  hasPermssion: 0,
  credits: 'reiko dev',
  description: 'Talk to Mistral AI',
...
event.messageID);
    const response = await axios.get(`https://www.api.vyturex.com/mistral?input=${encodeURIComponent(input)}`);
    const sagot = response.data || 'No Answers Found!';

    api.sendMessage(`𝗠𝗜𝗦𝗧𝗥𝗔𝗟 𝗔𝗜🤖:\n\n${sagot}\n\n𝗗𝗲𝘃 𝗟𝗶𝗻𝗸: https://www.facebook.com/100081201591674\n\n𝗕𝘂𝘆 𝗠𝗲 𝗔 𝗖𝗼𝗳𝗳𝗲𝗲!☕\nhttps://reikodev.gumroad.com/l/codebox4chan\n\n${manilaTime}`, event.threadID, (error) => {
      if (!error) {
