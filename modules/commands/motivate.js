...
module.exports.run = async function ({ api, event, args }) {
  try {
    const category = args[0] || 'life'; // Life is the default category, p'wede mong baguhin (available categories: love and self)
    const jsonLink = 'https://raw.githubusercontent.com/Augustquinn/JSONify/main/motivate.json';

    const response = await axios.get(jsonLink);
    const motivateData = response.data;

    if (motivateData[category]) {
      const messages = motivateData[category];
      const randomMessage = messages[Math.floor(Math.random() * messages.length)];

      const randomBackgroundLink = backgroundLinks[Math.floor(Math.random() * backgroundLinks.length)];

      const background = await loadImage(randomBackgroundLink);
      ...
      fs.writeFileSync('motivate.jpg', imageBuffer);

      api.sendMessage(
        `⭐ 𝗥𝗔𝗡𝗗𝗢𝗠 𝗠𝗢𝗧𝗜𝗩𝗔𝗧𝗜𝗢𝗡 \n\n𝗖𝗔𝗧𝗘𝗚𝗢𝗥𝗬: ${category.toUpperCase()}\n\n  ⌲ ${randomMessage}`,
        event.threadID,
        event.messageID
      );
      ...
      await api.sendMessage(
        {
          attachment: fs.createReadStream('motivate.jpg'),
          body: 'Motivate Image result:',
        },
        event.threadID
      );
      fs.unlinkSync('motivate.jpg');
    } else {
      api.sendMessage('Invalid category.\n\nAvailable categories:\n   ⌲ life\n   ⌲ self\n   ⌲ love', event.threadID, event.messageID);
    }
  } catch (error) {
    console.error(error);
    api.sendMessage('An error occurred while fetching motivational messages.', event.threadID, event.messageID);
  }
};
