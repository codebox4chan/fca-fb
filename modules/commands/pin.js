...
module.exports.config = {
  ...
  description: "Pinterest photos",
  commandCategory: "image",
  usages: "<input> - 1-8 limit",
  usePrefix: true,
  cooldowns: 30
};
...
module.exports.run = async function ({ api, event, args }) {
  ...
  function callback(error, response, body) {
    if (error) {
      console.error(error);
      return api.sendMessage("𝗦𝗲𝗿𝘃𝗲𝗿𝗕𝘂𝘀𝘆!", event.threadID);
    }
    ...
    if (response.statusCode === 200) {
      ...
      Promise.all(imgabc)
        ...
        .catch(err => {
          console.error(err);
          api.sendMessage("Server Busy Please Try Again Later!", event.threadID);
        });
    } else {
      console.error(`Request failed with status code ${response.statusCode}`);
      api.sendMessage("Server is Currently Busy!", event.threadID);
    }
  }
