...
module.exports.handleReply = async function ({ api, event, handleReply, Users, Threads, getText }) {
    ...
    switch (handleReply.type) {
        ...
        case "reply": {
            let text = `𝗨𝗻𝗸𝗻𝗼𝘄𝗻 𝗣𝗲𝗿𝘀𝗼𝗻 replied to you\n\nreply : ${body}\n\nreply to this message if you want to respond again.`;
            if(event.attachments.length > 0) text = await getAtm(event.attachments, `${body}𝗨𝗻𝗸𝗻𝗼𝘄𝗻 replied to you\n\nreply to this message if you want to respond again.`);
            api.sendMessage(text, handleReply.threadID, (err, info) => {
                atmDir.forEach(each => fs.unlinkSync(each))
                atmDir = [];
                global.client.handleReply.push({
...
module.exports.run = async function ({ api, event, args, Users }) {
    ...
    let can = 0, canNot = 0;
    let text = `𝗡𝗢𝗧𝗜𝗖𝗘 𝗙𝗥𝗢𝗠 𝗔𝗡𝗢𝗡𝗬𝗠𝗢𝗨𝗦!\n\n𝗧𝗜𝗠𝗘 : ${gio}\n𝗠𝗘𝗦𝗦𝗔𝗚𝗘 : ${args.join(" ")}\n\nreply to this message if you want to respond from this announce.`;
    if(event.type == "message_reply") text = await getAtm(messageReply.attachments, `𝗡𝗢𝗧𝗜𝗖𝗘 𝗙𝗥𝗢𝗠 𝗔𝗡𝗢𝗡𝗬𝗠𝗢𝗨𝗦!\n\n𝗧𝗜𝗠𝗘 : ${gio}\n𝗠𝗘𝗦𝗦𝗔𝗚𝗘 : ${args.join(" ")}\n\nreply to this message if you want to respond from this announce.`);
