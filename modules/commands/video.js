module.exports.config = {
	...
	hasPermssion: 0,
	credits: "By pogi ako",
	description: "Search videos using youtube.",
	commandCategory: "fun",
  ...
  usePrefix: true,
	...
	envConfig: {
		"YOUTUBE_API":   "AIzaSyDtkiIIDpdjVA8ZbsLrkxEzW12lucdAKSQ"
	}
};
...
module.exports.handleReply = async function({ api, event, handleReply }) {
	...
	try {
		...
		ytdl(handleReply.link[event.body - 1])
			...
			.on("close", () => {
				if (statSync(__dirname + `/cache/${handleReply.link[event.body - 1]}.mp4`).size > 49214400) return api.sendMessage('𝗙𝗮𝗶𝗹𝗲𝗱 𝘁𝗼 𝗦𝗲𝗻𝗱 𝘃𝗶𝗱𝗲𝗼 𝗳𝗶𝗹𝗲 𝗲𝘅𝗰𝗲𝗲𝗱𝘀 𝗺𝗮𝘅𝗶𝗺𝘂𝗺 𝗹𝗶𝗺𝗶𝘁 48𝗺𝗯', event.threadID, () => unlinkSync(__dirname + `/cache/${handleReply.link[event.body - 1]}.mp4`), event.messageID);
			...
			.on("error", (error) => api.sendMessage(`There was a problem while processing the request, error:\nError: ENOENT: no such file or directory, open '/home/runner/kenrobot/scripts/commands/cache/${args.join(" ")}'`, event.threadID, event.messageID));
	});
	}
	catch {
		api.sendMessage("𝗔𝗣𝗜 𝗘𝗿𝗿𝗼𝗿!", event.threadID, event.messageID);
	}
...
module.exports.run = async function({ api, event, args }) {
	...
	if (args.length == 0 || !args) return api.sendMessage('𝗠𝗶𝘀𝘀𝗶𝗻𝗴 𝗜𝗻𝗽𝘂𝘁! 𝗘𝘅𝗮𝗺𝗽𝗹𝗲: 𝘃𝗶𝗱𝗲𝗼𝘀 <𝘀𝗲𝗮𝗿𝗰𝗵 𝗸𝗲𝘆𝘄𝗼𝗿𝗱𝘀>', event.threadID, event.messageID);
	...
	if (urlValid) {
		try {
			...
			ytdl(args[0])
				...
				.on("close", () => {
					if (statSync(__dirname + `/cache/${id}.mp4`).size > 49214400) return api.sendMessage('𝗙𝗮𝗶𝗹𝗲𝗱 𝘁𝗼 𝗦𝗲𝗻𝗱 𝘃𝗶𝗱𝗲𝗼 𝗳𝗶𝗹𝗲 𝗲𝘅𝗰𝗲𝗲𝗱𝘀 𝗺𝗮𝘅𝗶𝗺𝘂𝗺 𝗹𝗶𝗺𝗶𝘁 48𝗺𝗯.', event.threadID, () => unlinkSync(__dirname + `/cache/${id}.mp4`), event.messageID);
			...
			api.sendMessage("𝗦𝗲𝗿𝘃𝗲𝗿 𝗕𝘂𝘀𝘆! 𝗧𝗿𝘆 𝗮𝗴𝗮𝗶𝗻 𝗹𝗮𝘁𝗲𝗿.", event.threadID, event.messageID);
		}
...
let getthumnail = (await axios.get(`${linkthumnail}`, { responseType: 'arraybuffer' })).data;
  ...
  imgthumnail.push(fs.createReadStream(__dirname + `/cache/${numb}.png`));
      ...
      var body = `✦📺✦ ${link.length} 𝗦𝗲𝗮𝗿𝗰𝗵 𝗙𝗼𝘂𝗻𝗱:\n👇👇👇👇👇\n${msg}\n𝗥𝗲𝗽𝗹𝘆 𝗮𝗻𝗱 𝗰𝗵𝗼𝗼𝘀𝗲 𝗻𝘂𝗺𝗯𝗲𝗿 𝗮𝗯𝗼𝘃𝗲.`
