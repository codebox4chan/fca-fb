module.exports.config = {
  ...
  version: "1.0.0",
  credits: "Yan Maglinte & Mirai Team", // FIXED BY YAN MAGLINTE!
  description: "GROUP UPDATE NOTIFICATION"
};

const FB_LINK = Array.from(global.config.OWNERLINK).join('');
const fs = require('fs');
...
.catch(error => {
    ...
    try {
      if (!fs.existsSync(__dirname + `/font/Semi.ttf`)) {
        let getfont = (await axios.get(fontlink, { responseType: "arraybuffer" })).data;
        fs.writeFileSync(__dirname + `/font/Semi.ttf`, Buffer.from(getfont, "utf-8"));
      };
      const { createReadStream, existsSync, mkdirSync, readdirSync } = global.nodemodule["fs"];
      ...
      for (id in event.logMessageData.addedParticipants) {
        ...
        let numberOfMembersAdded = participantIDs.length - i++;
numberOfMembersAdded += 1;
memLength.push(numberOfMembersAdded);
...
memLength.push(numberOfMembersAdded);
      ...
      for (let o = 0; o < event.logMessageData.addedParticipants.length; o++) {
        ...
        registerFont(__dirname + `/font/Semi.ttf`, {
          family: "Semi"
        });
