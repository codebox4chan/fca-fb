...
api.changeNickname(`${(!global.config.BOTNAME) ? "kennethpanio" : global.config.BOTNAME}`, threadID, api.getCurrentUserID());
		/*return api.sendMessage(`AI connected successfully\n\nabout me?\n Name : ${global.config.BOTNAME}\nPrefix : ${global.config.PREFIX}\ndescription : ${global.kennethpanio.DESCRIPTION}\n\nhow to use?\n${global.config.PREFIX}help (command list)\n${global.config.BOTNAME} (question) - no prefix\ntalk (text) - no prefix\n\nthankyou for using ${global.config.BOTNAME} BOT.`, threadID);
	}
	else {*/
		try {
